...
Console.WriteLine("########################################################");
Console.WriteLine("##                                                    ##");
Console.WriteLine("##                    SWITCHARONI                     ##");
Console.WriteLine("##                                                    ##");
Console.WriteLine("##              by: M. Wilson 8/2025                  ##");
Console.WriteLine("##                                                    ##");
Console.WriteLine("##    - Enter the Management IP {###.###.###.###}     ##");
Console.WriteLine("##       of the Cisco Switch that you want to get     ##");
Console.WriteLine("##       all of the IP addresses of hosts connected   ##");
Console.WriteLine("##       switch ports.                                ##");
Console.WriteLine("##                                                    ##");
Console.WriteLine("##    - You will be prompted for the ssh USERNAME     ##");
Console.WriteLine("##       and PASSWORD for the switch.                 ##");
Console.WriteLine("##                                                    ##");
Console.WriteLine("########################################################");
Console.WriteLine(" ");
Console.ResetColor();
...
CiscoSwitchManager switchManager = new CiscoSwitchManager(_switchip, _username, _password);
await switchManager.RunCommandsAsync(_commandsToRunOnSwitch);

// remove last two chars from the InterfaceName property of each Interface
//foreach (var mac in switchManager.MacAddresses)
//{
//    if (mac.Port.Length > 2)
//    {
//        if(mac.Port.Substring(mac.Port.Length - 2) == "\r")
//        {
//            mac.Port = mac.Port.Substring(0, mac.Port.Length - 2);
...
public class CiscoSwitchManager
{
    private readonly string _ipAddress;
    private readonly string _userName;
    private readonly string _passWord;
    public CiscoSwitchManager(string ipAddress, string username, string password)
    {
    ...
    public async Task RunCommandsAsync(List<string> commandsToRunOnSwitch)
    {
        ...
        try
        {
            ...
            Console.WriteLine("Connection Successful, Bruh!");
            Console.ResetColor();
            Console.WriteLine();
            ...
            using (var shellStream = sshClient.CreateShellStream("vt100", 80, 24, 800, 600, 1024))
            {
                ...
                foreach (string command in commandsToRunOnSwitch)
                {
                    ...
                    else if(command == "show ip arp")
                    {
                        Console.WriteLine("Infiltrating  arp data....");
                    }
                    else if (command == "show mac address-table dynamic")
                    {
                        Console.WriteLine("Stealing entries from mac address table....");
                    }
                    ...
                    foreach (var l in result.Split('\n'))
                    {
                        ...
                        else if (l.Contains("DYNAMIC"))
                        {
                            ...
                            {
                                macAddr.Mac = parts[1];
                                macAddr.Port = parts[3]; // 3 wokrs on 9300 but not 4507
        ...
        finally
        {
            ...
            Console.WriteLine();
            Console.ForegroundColor = ConsoleColor.Green;
            Console.WriteLine("....Disconnected, Bruh!");
            Console.ResetColor();
            Console.WriteLine();
        }
    }
